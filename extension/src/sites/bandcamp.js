...
(function () {
  ...
  function getMetadata() {
    ...
    //
    // The userscript read this off `unsafeWindow`, Tampermonkey's handle on the
    // page's own globals. A content script has no such handle — it runs in an
    // isolated world, where `window` is not the page's — so that branch was
    // dead on arrival and is removed rather than left as a path that can never
    // be taken. The `data-tralbum` attribute carries the same object and lives
    // in the DOM, which the isolated world does share.
    let tralbum = null;
    try {
      const dataEl = document.querySelector('[data-tralbum]');
      if (dataEl) tralbum = JSON.parse(dataEl.getAttribute('data-tralbum'));
    } catch { /* malformed attribute — the DOM scrape below still applies */ }
